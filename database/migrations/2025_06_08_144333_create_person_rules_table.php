...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('person_rules', function (Blueprint $table) {
            ...
            $table->longText('rule_history')->comment('سوابق قبلی');
            ...
            $table->string('rule_person_id')->comment('نماینده درخواست کننده')->index();
            $table->string('rule_president_id')->comment('رئیس جمهور')->index();
            $table->string('rule_gov_period_id')->comment('شماره دولت')->index();
            $table->string('rule_parliament_period_id')->comment('دوره مجلس')->index();
            $table->timestamps();
            $table->softDeletes();
        });
    }
