...
return new class extends Migration {
    ...
    public function up(): void{
        Schema::create('person_deputy_governor_trip', function (Blueprint $table) {
            $table->comment('سفرهای استانی معاون استان');
            $table->id('trip_id')->autoIncrement();
            $table->string('trip_president_id')->comment('رئیس جمهور')->index();
            $table->string('trip_person_id')->comment('نام نماینده')->index();
    ...
    {
        Schema::dropIfExists('person_deputy_governor_trip');
    }
};
